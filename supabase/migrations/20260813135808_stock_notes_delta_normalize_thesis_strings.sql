-- 既存データに想定外のJSONB形状があっても、画面表示用の差分レスポンスは文字列配列を返す。
...
with normalized_known as (
  select case
    when jsonb_typeof(coalesce(p_known_manifest, '{}'::jsonb)) = 'object' then coalesce(p_known_manifest, '{}'::jsonb)
    else '{}'::jsonb
  end as manifest
),
bundles as (
    ...
    jsonb_build_object(
      ...
      'theses', coalesce((
        select jsonb_agg(jsonb_build_object(
          ...
          'thesis', coalesce((select jsonb_agg(item order by ord) from jsonb_array_elements(case when jsonb_typeof(t.thesis) = 'array' then t.thesis else '[]'::jsonb end) with ordinality as e(item, ord) where jsonb_typeof(item) = 'string'), '[]'::jsonb),
          'risks', coalesce((select jsonb_agg(item order by ord) from jsonb_array_elements(case when jsonb_typeof(t.risks) = 'array' then t.risks else '[]'::jsonb end) with ordinality as e(item, ord) where jsonb_typeof(item) = 'string'), '[]'::jsonb),
          'nextCheck', coalesce((select jsonb_agg(item order by ord) from jsonb_array_elements(case when jsonb_typeof(t.next_check) = 'array' then t.next_check else '[]'::jsonb end) with ordinality as e(item, ord) where jsonb_typeof(item) = 'string'), '[]'::jsonb),
