...
import { parseYutaiBenefitSummary, type YutaiBenefitSummary } from "./benefit-summary";

type State = { status:"idle"|"signed_out"|"loading"|"ready"|"error"; data:YutaiBenefitSummary|null; error:string|null };

export function useYutaiBenefitSummary(today: string) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const {data,error} = await client.rpc("stock_notes_get_yutai_benefit_summary_v1",{p_today:today});
      if (error) throw error;
      setState({status:"ready",data:parseYutaiBenefitSummary(data),error:null});
    } catch (error) {
