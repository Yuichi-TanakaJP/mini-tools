...
- これは業務データの復元UIであり、復元先設定・実端末UAT・切替は別工程。2026-09-11に本番へAPIを設置し、既存11テーブルの全件fingerprint不変を確認した。復元先設定は空で、復元の実行はまだ無効。通常復元欄では旧形式を自動変換しない。

## 従来の端末バックアップ

復元欄の「旧優待データを追加取込」では期限帳v2配列、または端末バックアップ内の
メモ・タグ・権利年月付き仕込み/アーカイブ・月別手入力・共通ピック/パス・期限帳v2を追加案へ変換する。
不明な追加フィールド、権利年欠落、重複候補、既存との競合は停止する。他ツールのキーは送信しない。
期限帳の全履歴・残高・日付を検査し、取込IDは安定生成する。同じファイルの再入力で二重追加しない。
差分確認後は通常復元と同じ変更前バックアップ・同意・原子的保存・全件再照合を経る。
既存行は内容を保持するが、復元APIの仕様によりrevision/updated_atは更新される。
変換後〜プレビュー間の他端末追加/編集は、削除や業務内容の変更を含むプランを拒否して保護する。
読込は明示された欄だけで行い、旧端末ストレージへ書き戻さない。古い未対応形式を黙って丸めない。

既存LocalStorage版は維持する。`mini-tools-localstorage-backup` 形式を出力し、merge/replaceで端末へ取り込む。
優待DB専用形式は従来parserで拒否し、端末形式は通常のDB照合parserで拒否する。変換は上記「旧優待データを追加取込」で明示選択した場合だけ行う。
