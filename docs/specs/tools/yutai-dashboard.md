...
- 12ヶ月ビュー: 登録済みメモを対象にした 銘柄 × 12 ヶ月のガント風グリッド。仕込み開始（仕）〜権利月（権）を帯で表示し、取得済み権利月に ✓を付ける。帯は年をまたいで循環。検索・クロス戦略フィルタが効く。日興 / SBI などの当日変動値は載せない
  ...
  - **取得済み**: 選択年度に取得した権利月は緑✓、選択年度以外に取得実績がある月は灰✓（過去取得）。ツールチップに取得年を列挙。現在の取得は、取得済みにした時点から次に到来する権利月を固定した `acquiredEntitlementMonthKey` で表示する（[2026-08-07 次回権利月の固定](../../decision-log/2026-08-07-yutai-acquired-entitlement-lock.md)）
...
- 一覧テーブル: コード / 銘柄 / 権利月 / 簡易効率 / 長期 / 日興 / SBI / 仕込み開始 / 1株開始 / クロス戦略 / 実績 / 操作
  - 実績列は未アーカイブの今回取得があれば `✓ YYYY-MM` を過去履歴より優先表示し、過去履歴の件数はツールチップに残す
  - 詳細パネルに今回の対象権利年月を表示し、誤判定時だけ「変更」から候補を選び直せる
  - 「簡易効率」列は 2段構成。1段目が簡易優待効率（%）、2段目が日興クロス取引の概算手数料（[2026-07-26 決定](../../decision-log/2026-07-26-yutai-dashboard-cross-fee.md)）
  - 「長期」列は 1 列で 2 種のチップを出す。「長」= 全 tier が長期保有必須（短期では優待なし）、「特」= 長期保有が条件の tier がある（長期保有で優待・特典）。両方該当すれば併記。公式条件（launch-display）の tier 保有月数から導出（選択月のみ。全月表示では公式条件データ非取得のため空欄）
  - 列幅: 銘柄列を固定幅にし、余白は末尾のスペーサ列で吸収する（PC の広い画面で銘柄列が間延びしないように）
...
- 表のセルからの直接編集: クロス戦略・仕込み開始・1株保有開始をセルをクリックしてその場で変更・保存できる。未登録の候補行でも編集でき、確定時に自動で優待メモへ追加する（先に「＋メモ」を押す必要はない）
  ...
  - クロス戦略のみ、メモが無くても公式条件（launch-display）に長期特典が無いと判定できた銘柄は「長期優遇なし」を自動導出して控えめに表示し、クロス戦略フィルタでも同じ導出値で絞り込める。公式レコードが無い（不明）銘柄や長期特典あり未トリアージの銘柄は「未設定」のまま残す。導出はあくまで表示・絞り込み用でメモは保存しない（参照: [戦略欄の長期優遇なし自動導出](../../decision-log/2026-07-29-yutai-dashboard-derive-cross-strategy.md)）
...
- 手数料 = 買方金利（買い）＋ 貸株料（売り）。各 = 約定金額 × 年率 × 日数 ÷ 365。約定金額は簡易効率の必要資金（株価 × 必要株数）を使う
- 買い: 制度信用買い → 現引き（信用委託手数料・現引き手数料は 0 円）。現引きすると現物になり以降は金利がかからないため、買方金利は**現引きまでの日数（既定=2営業日相当の暦日）分だけ**計上する。保有全期間には課金しない
- 売り: 一般信用売建の対象銘柄なら在庫状況によらず一般で計算する（在庫0・取引停止でも一般前提）。一般の対象外の銘柄に限り奥の手として制度信用売り。売り建ては現渡しの受渡（返却）まで継続するため、貸株料は**今日→返却日分**を計上する。制度信用売り時のみ「＋逆日歩（別途）」をツールチップに注記し、逆日歩自体は金額に含めない
- 日数（JPX 休場日カレンダーで算出）:
  - 貸株料は 今日（JST）→ **返却日**（現渡しの受渡日）の暦日数。返却日 = 権利付最終日（月末最終営業日の 2 営業日前）＋ 3 営業日（権利落ち +1、現渡し受渡 T+2 +2）。**月末が金曜だったり年末を跨ぐと、営業日カウントで返却日が後ろにずれ、貸株料日数が自然に増える**
  - 買方金利は 今日 → 現引き（既定=2 営業日）までの暦日数
  - 権利確定日は月末締め前提の近似で、非月末締め銘柄はズレる（対象外）
- 率は `_shared/yutai-cross-fee.ts` の `NIKKO_CROSS_FEE_RATES` に集約（2026-07 時点: 制度買方金利 2.54% / 制度貸株料 1.15% / 一般貸株料 1.90%）。改定時はここだけ更新する
- 表示: 2段目は金額のみ `≈¥XXX`（幅を抑えるためラベルやインライン注記は付けない）。内訳・保有日数・権利付最終日・逆日歩・買建/売建不可の注記はすべて title ツールチップに寄せる。制度信用買い不可でも買方金利は参考値として計算し、一般・制度とも売建不可でも一般前提で概算し、**常に計算結果を出す**。候補行かつ株価・必要株数・日興データが揃うときだけ表示する
- 1段目の効率%チップの色: 手数料が算出できる行では「優待価値 − 手数料」の損益で色分けする。プラスなら青系、0以下なら赤系。手数料が出せない行は従来色（橙）。損益の内訳は効率チップの title ツールチップに表示する
