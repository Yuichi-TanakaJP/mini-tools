...
- 12ヶ月ビュー: 登録済みメモを対象にした 銘柄 × 12 ヶ月のガント風グリッド。仕込み開始（仕）〜権利月（権）を帯で表示し、取得済み権利月に ✓を付ける。帯は年をまたいで循環。検索・クロス戦略フィルタが効く。日興 / SBI などの当日変動値は載せない
  ...
  - **取得済み**: 選択年度に取得した権利月は緑✓、選択年度以外に取得実績がある月は灰✓（過去取得）。ツールチップに取得年を列挙。取得実績の年度は取得日から逆算され、仕込み開始〜権利月のリード期間中に取得した場合は当年の権利月に寄せる（[2026-07-12 取得実績のひもづけ](../../decision-log/2026-07-12-yutai-entitlement-attribution-lead-time.md)）
  - **1株保有**: 選択年度で解釈する。選択年度 > 開始年=通年、= 開始年=開始月〜12月（開始月マーカー）、< 開始年=非保有。開始が YYYY-MM で判定できないフリーテキストは年不明として通年扱い。銘柄名の下に「1株 YYYY-MM〜（N年目）」を表示する
  - **仕込み実績**: 実際にクロス/現物を仕込んだ年月を記録・表示する（計画の帯/仕とは別のフクシア●）。月セルをクリックすると選択年度のその月の仕込み実施をトグルする。ダッシュボード独立の LocalStorage（`yutai_dashboard_prep_log_v1`）で per-銘柄に持つ（[2026-07-12 仕込み実績記録](../../decision-log/2026-07-12-yutai-dashboard-prep-log.md)）。断念記録（取引停止・残数なし）は別途仕様化予定
