...
- 12ヶ月ビュー: 登録済みメモを対象にした 銘柄 × 12 ヶ月のガント風グリッド。仕込み開始（仕）〜権利月（権）を帯で表示し、取得済み権利月に ✓（緑）を付ける。帯は年をまたいで循環。検索・クロス戦略フィルタが効く。日興 / SBI などの当日変動値は載せない
  ...
  - 1株保有: `oneShareStartedAt` があれば通年継続とみなし、下レーンに保有ストリップ（薄緑）を全月に引く。開始月（YYYY-MM から判定）は濃い緑で強調し、銘柄名の下に「1株 YYYY-MM〜」を表示する
  - 重なりの表示: 取得済み・仕込み開始・権利は優先度（取得✓ ＞ 権 ＞ 仕）で主バッジを決める。権利月と仕込み開始が同月に重なる場合は主バッジ右上に橙のドットを添える（クロスの当月仕込みなどで発生）
