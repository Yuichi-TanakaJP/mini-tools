...
- 売り: 一般信用売建の対象銘柄なら在庫状況によらず一般で計算する（在庫0・取引停止でも一般前提）。一般の対象外の銘柄に限り奥の手として制度信用売り。売り建ては現渡しまで継続するため、貸株料は**保有期間（今日→権利付最終日）分**を計上する。制度信用売り時のみ「＋逆日歩（別途）」をツールチップに注記し、逆日歩自体は金額に含めない
- 日数: 貸株料は 今日（JST）→ 権利付最終日（権利月の月末最終営業日の 2 営業日前）の暦日数。買方金利は 今日 → 現引き（既定=2営業日）までの暦日数。権利確定日は月末締め前提の近似で、非月末締め銘柄はズレる（対象外）
