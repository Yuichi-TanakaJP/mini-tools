...
- DB接続の隔離検証フラグ有効時は個人データをSupabaseから取得・保存する。末尾の「DB接続の隔離検証」を参照。
...
## DB接続の隔離検証（2026-09-10）

`NEXT_PUBLIC_YUTAI_DASHBOARD_DB_PREVIEW=true` の場合のみ、個人データをSupabase共通repositoryから読み書きする。
未設定/falseでは従来のLocalStorage版を維持する。本番切替は未完了。premium認証・市場データ取得経路は変更しない。

- 表・全月表示・12か月表示を維持し、個人データはowner単位の全件workspaceを利用する。全月表示でも12回取得は不要。
- ピック/パスは行の権利月に保存。月別の未選択はglobal値の復活を抑止する。既存のglobal選択は変更しない。
- 仕込み月軸は月別設定ごとに1行。全月表示でも銘柄単位の仕込み時期を他月へ流用しない。
- 年間表示の仕込み計画も各権利月ごとに計算。仕込み済み履歴は対象年/月を保持し、実施月の点は実際のJST実施年へ表示する。月登録解除後の履歴も保持する。
- 銘柄編集は方針・優先度・メモ・1株開始などを保存。月別仕込み時期・必要株数・優待価値は月別テーブルへ保存。
- 株数・価値は入力を離れた時に保存し、毎キー入力では送信しない。価値は小数/0を保持、空欄はnull、株数は正の整数。
- 詳細の取得済みチェックは行の対象年/月だけを編集する。別年/月の履歴編集は優待メモの履歴操作で行う。権利確保後の履歴をこのチェックで巻き戻さない。
- メモから外す操作は対象の月登録だけ解除し、銘柄・他月・仕込み履歴・残高を削除しない。実行前に確認する。
- 編集開始時のrevisionを保持。競合時は最新データを確認して編集し直す。応答喪失は同じ要求IDで再確認し、未確定中の操作・離脱を止める。
- DB設定不足/未ログイン/読取失敗時は旧データへfallbackしない。DBモードでは旧キーを読まない・更新しない。

関連: [UAT](../../uat/yutai-dashboard.md)、[判断記録](../../decision-log/2026-09-10-yutai-dashboard-db.md)、[全6工程](../cross-cutting/yutai-repository.md)。
上記の既存仕様でLocalStorageに言及する箇所は、検証フラグが無効な場合の動作。
