...
- 12ヶ月ビュー: 登録済みメモを対象にした 銘柄 × 12 ヶ月のガント風グリッド。仕込み開始（仕）〜権利月（権）を帯で表示し、取得済み権利月に ✓（緑）を付ける。帯は年をまたいで循環。検索・クロス戦略フィルタが効く。日興 / SBI などの当日変動値は載せない
  - 各セルは上下2レーン構成。上レーンが優待クロスのサイクル（帯 / 仕 / 権 / ✓）、下レーンが1株保有ストリップ
  - 1株保有: `oneShareStartedAt` の開始月（YYYY-MM）から 12 月まで下レーンに保有ストリップ（薄緑）を引き、開始月を濃い緑で強調する。開始前の月は塗らない。開始月が判定できないフリーテキストのみ通年保有として全月に引く。銘柄名の下に「1株 YYYY-MM〜」を表示する
  - 重なりの表示: 権利月は取得の有無に関わらず常に「権」で示し、取得済みは主バッジ右上の緑✓、権利月と仕込み開始が同月に重なる場合は左上の橙ドットで重ねて示す（取得済み・権利・仕込みを区別できる）
