...
- 一覧テーブル: コード / 銘柄 / 権利月 / 簡易効率 / 長期要 / 長期特 / 日興 / SBI / 仕込み開始 / 1株開始 / クロス戦略 / 実績 / 操作
  - 「簡易効率」列は 2段構成。1段目が簡易優待効率（%）、2段目が日興クロス取引の概算手数料（[2026-07-26 決定](../../decision-log/2026-07-26-yutai-dashboard-cross-fee.md)）
  - 「長期要」= 全 tier が長期保有必須（短期では優待なし）、「長期特」= 長期保有が条件の tier がある（長期保有で優待・特典）。いずれも公式条件の tier 保有月数から導出（選択月のみ。全月表示では公式条件データ非取得のため空欄）
