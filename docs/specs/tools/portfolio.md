...
- **DB確認**: Supabaseから取得したportfolio、snapshot、position、review、review itemの件数・ID・基準日を読み取り専用で確認する
...
- DBへの保存、更新、削除は行わない
- 画面に表示する件数は、現在読み込んだSupabase結果を集計したもの。未取込・認証必須・ready snapshotなしを0件の成功状態と混同しない
