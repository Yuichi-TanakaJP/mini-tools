...
| 操作 | 挙動 |
|---|---|
| ＋ / − ボタン | 図の中心を固定して 1.4 倍ずつ拡大・縮小。現在の倍率を%で表示する |
| ⟲ ボタン | 等倍・移動なしへ戻す |
| ドラッグ | 図を移動する。しきい値（6ユーザー座標）を超えた移動のあとは領域を選択しない |
| Ctrl / ⌘ + ホイール | ポインタ位置を固定して拡大縮小する |
| ホイール単体 | 拡大縮小しない。ページの縦スクロールに使う |
| ダブルクリック | その位置を固定して1段階拡大する |
| ピンチ | 2本指の中点を固定して拡大縮小する |

倍率は 0.5〜8 倍で頭打ちにする。domainを切り替えると等倍へ戻る。

タッチ操作は、等倍のあいだ `touch-action: pan-y`（1本指で画面を縦スクロールできる）、拡大後は `touch-action: none`（図の操作を優先する）へ切り替える。⟲ で等倍に戻すとページのスクロールに戻る。
...
| `app/premium/industry-map/use-pan-zoom.ts` | 図の拡大縮小・移動。計算部分は純関数 |
