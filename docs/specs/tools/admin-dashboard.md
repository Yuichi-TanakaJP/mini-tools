...
- robots: `noindex, nofollow` (manifest 取得が走るため `dynamic = "force-dynamic"`)
...
| `weekly-task` | 決算カレンダー 国内/海外 (weekly task) | 8 |
| `monthly-start` | 優待カレンダー / 市場ランキング 時価総額・配当利回り | 35 |
| `manual-wrapper` | TOPIX33 / 日経寄与度 / 株価ランキング (run_naito_and_backup.ps1) | null (SLA 対象外) |
| `ad-hoc` | 米国株ランキング / EDINET / TDNET / JPX 祝日 (運用要確認) | null |
| `user-input` | ローカル系 (合計計算 / 文字数カウント / 優待期限帳 / 優待メモ) | null |
...
age は `(now - row.latest)` を YYYY-MM-DD ベースで日数換算 (UTC、週末・祝日オフセットなし)。
...
- ページ自体は `dynamic = "force-dynamic"` だが、`fetch()` ごとの Data Cache は revalidate 設定を尊重するため、10 分以内の連続アクセスは **API を再び叩かない**
