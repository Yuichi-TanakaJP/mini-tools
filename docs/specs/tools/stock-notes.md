...
1. **マイ銘柄リストからの一括取り込みバナー**（タブの外）: `my_stocks_items_v1` の `tab === "holding"` のうち `stock_notes_stocks` に未登録のものが1件以上あるとき、「マイ銘柄リストに未登録の保有が N 件あります」と表示する。「まとめて登録」を押すと、押した時点の対象一覧を**スナップショットとして固定**したうえで展開表示し（`bulkImportSnapshot`。開いた後に裏の再取得や保有リスト更新で対象が変わっても、確認画面と実際の登録対象がずれないようにするため。詳細は decision-log 参照）、確認したうえで「N件を『保有』としてまとめて登録」を押すと `category='holding'` で一括登録する。いきなり書き込まず、対象一覧を確認できる2段階にする。失敗した銘柄があれば個別にメッセージを表示し、成功分はそのまま登録された状態を保つ。**全件成功して対象が0件になっても、結果（成功件数・失敗一覧）が表示されたままバナー自体は消えない**（閉じるまで表示を維持する。「実行したのに何も起きなかった」ように見えることを避けるため）。成功が0件でも「既に登録されています」（`23505`、他タブ等での先行登録）だけで終わった場合は、DB側には反映済みのため裏で再取得してキャッシュの「未登録」表示を更新する（`shouldRefetchAfterBulkImport`、`app/tools/stock-notes/logic.ts`）。詳細は「銘柄の登録・分類変更・アーカイブ・削除」節。**stock_notes_stocks に登録済みだが分析0件の銘柄はここには出さない**（「要対応」タブが分析0件を扱うため。このバナーは「行そのものが無い」銘柄だけを対象にする。旧「保有だが未分析」セクションを廃止した経緯は decision-log 参照）
...
| タブ | 対象 | 並び順 |
|---|---|---|
| 要対応（既定） | アーカイブを除く全銘柄のうち、次のいずれかに該当するもの: (1) 分析0件 (2) 最終分析日より後に決算があった（決算またぎ、鮮度バッジの `post-earnings` 判定と同じ） (3) 期限切れ・期限間近（7日以内、`ACTION_DUE_SOON_DAYS`）の未消化アクションがある | **保有（`category = holding`）を先頭に** → 次回決算日が近い順（未判明は後ろ）→最終分析日の古い順（未分析が最上位） |
...
キャッシュに銘柄ごとのRevision manifest（`stockId -> revision`）がある場合、再取得は `POST /api/stock-notes/delta` を優先する。サーバーはRLSで本人の4テーブルを対象に、画面表示用の銘柄本体・分析・見立て・未消化アクションを銘柄単位に正規化して内容ハッシュRevisionを計算する。Revisionが一致する銘柄の子データは返さず、変更・追加銘柄だけを返す。ハッシュは差分判定用で、認証や改ざん検知には使わない。
...
書き込み（登録・分類変更・アーカイブ・削除・一括取り込み）成功後は必ず `invalidateStockNotesCache(userId)` を呼び、`revalidateData` で再取得する（呼び忘れると、TTL内は古いキャッシュがそのまま表示され続け「登録したのに画面に反映されない」不具合になる）。

書き込み〜再取得の間にも認証状態の世代ガード（`authGuardRef`）を適用する。マウント時の `getUser()` と同じ非同期の隙間の問題が書き込み経路にもある: 書き込みハンドラは開始直前に `authGuardRef.current.current()` で認証世代を記録し（`ToolClient.tsx` の `submitRegister` / `handleCategoryChange` / `handleDeleteStock` / `runBulkImport`）、書き込み（INSERT/UPDATE/DELETE）が完了した後、`refetchAfterWrite` の中でこの世代が現在も current かを確認する。ユーザーが書き込み中に別アカウントへ切り替わっていた場合、世代が不一致になり再取得・キャッシュ保存を一切行わずに中止する。DBへのINSERT/UPDATE/DELETE自体はRLS（`auth.uid() = user_id`）が別ユーザーの行への書き込みを拒否するため守られているが、これだけでは「切替後のセッションで再取得したデータを、切替前のユーザーIDのキャッシュキーへ保存してしまう」というキャッシュのユーザー境界の破れを防げない。この認証世代ガードは既存の `createLoadGuard`（取得の世代ガードとして作られたもの）を再利用しており、`revalidateData` は省略可能な第3引数 `authToken` としてこれを受け取る。詳細は decision-log 参照。
