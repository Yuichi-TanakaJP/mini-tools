...
0. **Portfolio保有の状態**: audience APIで公式ready snapshotの国内株式を読み、件数・基準日を表示する。保有明細と全体方針はPortfolioへのリンクから確認する。
1. **分析未登録の保有件数**: Portfolio保有のうち分析対象に未登録の件数を表示する。必要な銘柄を通常フォームで登録する。旧マイ銘柄からの一括登録・CSV再取込は不要。
2. **銘柄一覧**: 5つのタブ（**要対応** / 保有 / ウォッチ / 新規調査 / アーカイブ）。既定表示は「要対応」。各タブのラベルには件数を表示する（例: 「要対応 15」）。行にコード・銘柄名・現在の見立て（強気/中立/弱気の色分け＋確信度）・最終分析日・分析件数・未消化アクション数・次回決算日の表示（下記）。「要対応」タブでは鮮度バッジの代わりに**理由バッジ**（下記）と「分析用プロンプトをコピー」ボタンを表示し、他の4タブでは鮮度バッジを表示する。行をタップすると詳細を展開（アコーディオン）。展開部の末尾に分類変更・削除の操作を置く。タブの右に「＋ 銘柄を登録」ボタンがあり、証券コード・銘柄名・分類を入力して新規登録できる（詳細は「銘柄の登録・分類変更・アーカイブ・削除」節）
...
| タブ | 対象 | 並び順 |
|---|---|---|
| 要対応（既定） | アーカイブを除く全銘柄のうち、次のいずれかに該当するもの: (1) 分析0件 (2) 最終分析日より後に決算があった（決算またぎ、鮮度バッジの `post-earnings` 判定と同じ） (3) 期限切れ・期限間近（7日以内、`ACTION_DUE_SOON_DAYS`）の未消化アクションがある | **Portfolioで実保有している銘柄を先頭に** → 次回決算日が近い順（未判明は後ろ）→最終分析日の古い順（未分析が最上位） |
| 保有 | Portfolioの国内株式に含まれる登録銘柄 | 最終分析日の古い順（未分析が最上位） |
| ウォッチ | `category = watch` | 同上 |
| 新規調査 | `category = research` | 同上 |
| 新規調査（互換表示） | 旧 `category = holding` だがPortfolioでは非保有の銘柄 | 同上 |
...
**アーカイブでもPortfolioで実保有中なら保有・要対応の対象にする**。分類変更で実保有を隠さず、行に分類と保有を別々に表示する。

要対応タブの並び順で保有を最優先にしているのは、保有銘柄は実際に資金を投じており、ウォッチ・新規調査より確認の緊急度が高いため（詳細は decision-log 参照）。

### 要対応タブの理由バッジ

「要対応」タブの各行には、なぜその銘柄が要対応なのかを示す理由バッジを表示する（`actionRequiredReasons`、`app/tools/stock-notes/logic.ts`、純関数・ユニットテスト済み）。複数該当する場合はすべて表示する。

| バッジ | 条件 |
|---|---|
| 未分析 | 分析が0件 |
| {M/D}の決算後 | 最終分析日より後に決算があった（決算またぎ。`post-earnings` 判定と同じ、決算日を含めて表示する） |
| 期限切れ | 未消化アクションのうち期限が既に過ぎているものがある |
| 期限間近 | 未消化アクションのうち期限切れではないが `ACTION_DUE_SOON_DAYS`（7日）以内に迫っているものがある |

理由バッジは既存の鮮度バッジ（`FreshnessBadge`。そろそろ確認/要更新/要更新（決算後未分析））と同じ情報（決算またぎ）を2箇所に重複表示しないよう、**「要対応」タブでは鮮度バッジの代わりに理由バッジだけを表示し、他の4タブ（保有/ウォッチ/新規調査/アーカイブ）では従来どおり鮮度バッジを表示する**という使い分けにした。理由は decision-log 参照。

### 銘柄の登録・分類変更・アーカイブ・削除

- **登録**: 「＋ 銘柄を登録」から証券コード・銘柄名・分類（ウォッチ/新規調査。アーカイブは新規登録の選択肢に出さない）を入力して登録する。証券コードを入力すると、銘柄マスター（`app/api/stock-notes/stock-master/route.ts` 経由、`MARKET_INFO_API_BASE_URL` の `/stock-master/latest` を取得）から一致する銘柄名を自動補完する。マスターに無いコード（米国株等）は銘柄名欄を手入力すれば登録できる。既に登録済みのコードはクライアント側で事前チェックして登録させず、それをすり抜けた場合（`stock_notes_stocks` の `unique(user_id, code)` 制約違反）もエラーメッセージで案内する
- **分類変更**: 一覧行を展開すると分類セレクトがあり、watch/research/archived に変更できる（旧holdingは現在値としてのみ表示）。変更すると `category_changed_at` を更新時刻に更新する
...
- **削除**: 一覧行の展開部に削除ボタンがある。**分析が1件でもある銘柄は削除できない**（ボタンを無効化し、「分析N件が連鎖削除されるため削除できません。アーカイブしてください」と表示する）。この事前チェックはUX（押す前に理由を伝える）にすぎず、正しさの担保は**DB側の外部キー制約（restrict）**にある（`stock_notes_stocks` -> `stock_notes_analyses` / `stock_notes_theses` は stock-notes#15 で `on delete cascade` から `restrict` に変更済み、本番適用・実地検証済み）。表示中のキャッシュが古く事前チェックをすり抜けた場合でも、DBが `23503`（外部キー制約違反）を返して削除を拒否するため、`app/tools/stock-notes/data.ts` の `isForeignKeyViolationError` でこれを検出し、生のPostgRESTエラーではなく「この銘柄には分析が残っているため削除できません。アーカイブしてください」というメッセージに変換する。削除できるのは分析0件の銘柄だけで、実行前に確認ダイアログ（`window.confirm`）を出す

書き込み関数は `app/tools/stock-notes/data.ts` の `insertStock` / `updateStockCategory` /
`deleteStockById`。`user_id` はログイン中のユーザーIDを呼び出し側
...
銘柄一覧の各行に、次回決算に加えて前回決算も表示する。次回決算が未判明の銘柄では前回決算が唯一の決算関連の手がかりになるため、次回の判明状況によって表示の重みを変える。
...
保有は `/api/stock-notes/audience` でPortfolioと共通の選択処理を使う。Premium認証とSupabase認証・RLSが必要。`ready`、Portfolio未登録、snapshotなし、Premium未認証、取得失敗を区別する。分析・ウォッチは独立して利用できる。保有コードは永続キャッシュに保存しない。画面再取得時に現在の権限で読み直す。
...
キャッシュに銘柄ごとのRevision manifest（`stockId -> revision`）がある場合、再取得は `POST /api/stock-notes/delta` を参照。サーバーはRLSで本人の4テーブルを対象に、画面表示用の銘柄本体・分析・見立て・未消化アクションを銘柄単位に正規化して内容ハッシュRevisionを計算する。Revisionが一致する銘柄の子データは返さず、変更・追加銘柄だけを返す。ハッシュは差分判定用で、認証や改ざん検知には使わない。

- 完全なmanifestとの差分で存在しなくなった銘柄は、キャッシュと画面から削除する。これは `stock_notes_stocks` の行が消えた場合だけであり、`category = archived` では削除しない
- APIエラーや不正なレスポンスでは差分を適用せず、表示中のキャッシュを維持する
- 差分APIが失敗した場合は、従来のSupabase直接取得へフォールバックする。初回・旧形式キャッシュ（manifestなし）は差分APIから全銘柄を受け取り、manifestを作成する
- 決算情報と保有リストはSupabase銘柄差分とは別のデータ源なので、従来どおり個別に取得する
- 分析本文 `body` は一覧取得・Revision対象に含めず、「原文を表示」操作時に個別取得する

詳細な判断理由と、削除・部分障害・同時更新時の扱いは [2026-08-13 銘柄単位差分同期](../../decision-log/2026-08-13-stock-notes-delta-sync.md) を参照。
...
書き込み（登録・分類変更・アーカイブ・削除）成功後は必ず `invalidateStockNotesCache(userId)` を呼び、`revalidateData` で再取得する（呼び忘れると、TTL内は古いキャッシュがそのまま表示され続け「登録したのに画面に反映されない」不具合になる）。

書き込み〜再取得の間にも認証状態の世代ガード（`authGuardRef`）を適用する。マウント時の `getUser()` と同じ非同期の隙間の問題が書き込み経路にもある: 書き込みハンドラは開始直前に `authGuardRef.current.current()` で認証世代を記録し（`ToolClient.tsx` の `submitRegister` / `handleCategoryChange` / `handleDeleteStock`）、書き込み（INSERT/UPDATE/DELETE）が完了した後、`refetchAfterWrite` の中でこの世代が現在も current かを確認する。ユーザーが書き込み中に別アカウントへ切り替わっていた場合、世代が不一致になり再取得・キャッシュ保存を一切行わずに中止する。DBへのINSERT/UPDATE/DELETE自体はRLS（`auth.uid() = user_id`）が別ユーザーの行への書き込みを拒否するため守られているが、これだけでは「切替後のセッションで再取得したデータを、切替前のユーザーIDのキャッシュキーへ保存してしまう」というキャッシュのユーザー境界の破れを防げない。この認証世代ガードは既存の `createLoadGuard`（取得の世代ガードとして作られたもの）を再利用しており、`revalidateData` は省略可能な第3引数 `authToken` としてこれを受け取る。詳細は decision-log 参照。

### 入力

- 銘柄の登録（証券コード・銘柄名・分類）、分類変更・アーカイブ（分類・任意の理由）、削除（分析0件の銘柄のみ）。いずれも `stock_notes_stocks` への書き込みのみ
...
- 保有: `/api/stock-notes/audience`。Portfolioと同じ公式ready snapshotを選択し、国内株式のquantity > 0をコード単位で重複排除する。
...
- `stock_notes_stocks`（Supabase、RLS で本人の行のみ書き込み可）。登録・分類変更・アーカイブ・削除で書き込む
- `stock_notes_analyses` / `stock_notes_theses` / `stock_notes_actions` への書き込みは行わない（分析はGPTの領域）
- 旧マイ銘柄のデータは読み書きしない。既存の保存データは削除しない。
...
- 次回決算日（`/api/stock-notes/earnings`）の取得失敗は、他の4テーブル・保有リストの取得失敗とは別扱いにする。ダッシュボード本体（銘柄一覧・アクション受信箱、等）は決算日が無くても表示できるため、`app/tools/stock-notes/load.ts` の `loadDashboardData` はこの失敗だけを個別に catch し、`earnings: null` としてページ自体は正常表示する。決算日の表示箇所だけ「決算情報を取得できませんでした」になる（部分表示するのはこのケースのみ）
...
- 書き込み（登録・分類変更・アーカイブ・削除）が失敗した場合は、画面をエラー表示に倒さず、フォーム内または `window.alert` でエラーメッセージを表示する（表示中の一覧データは維持する）。重複コードの登録は事前チェック（`isCodeAlreadyRegistered`）に加え、それをすり抜けた場合の DB エラー（`unique(user_id, code)` 違反、`isDuplicateStockError` で判定）も専用のメッセージで案内する
- 分析が1件でもある銘柄の削除は、サーバーへのリクエスト自体を送らずクライアント側で防ぐ（削除ボタンを無効化）。ただしこれはUX上の事前案内であり、正しさの担保はDB側の外部キー制約（restrict）にある。事前チェックをすり抜けてリクエストが送られた場合も、DBが `23503` を返して拒否するため、`isForeignKeyViolationError`（`app/tools/stock-notes/data.ts`）で検出し「この銘柄には分析が残っているため削除できません。アーカイブしてください」と案内する（生のPostgRESTエラーは見せない）
...
| 分類タブ（保有/ウォッチ/新規調査/アーカイブ）に該当銘柄が0件 | 「この分類の銘柄はまだありません」（保有銘柄が1件も登録されていない場合も「保有」タブでこの文言になる。旧「保有だが未分析」セクション廃止に伴い、保有0件専用の文言は廃止した） |
| 要対応タブに該当銘柄が0件 | 「この分類の銘柄はまだありません」（他のタブと同じ文言を使う） |
...
| 分析が0件 | unknown | バッジなし（要対応タブでは代わりに理由バッジ「未分析」を表示する） |
...
旧category=holdingは互換データとして残し「旧分類: 保有」と表示するが、実保有の判定には使わない。手動分類の新規選択肢から保有を除く。旧一括登録・同期仕様から変更した理由は[統合判断](../../decision-log/2026-09-18-portfolio-holdings-authority.md)を優先する。
