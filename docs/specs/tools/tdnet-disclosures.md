...
- 検索設定プリセットはブラウザ `localStorage` の `tdnet_disclosures_search_presets_v1` に保存する
- 保存対象は、キーワード、カテゴリ、リンク種別、テーマ、時間帯、財務関連 / 決算短信 / 訂正除外の各フィルタ
- 日付と検索範囲はデータ取得条件なので、検索設定プリセットには含めない
- Supabase同期、スマホ・PC間同期、他ツール横断は今回のMVP対象外とする
...
| 検索設定なし | 保存済みの検索設定がないことを表示する |
| 検索設定保存 | 保存名が空なら保存せず、入力を促す |
| 既定設定あり | マウント後に localStorage から読み、既定設定の検索条件を復元する |
| 保存失敗 | localStorage例外を捕捉して失敗通知を表示する。画面をクラッシュさせず、失敗した変更は一覧にも反映しない |
...
- Decision Log: [2026-09-19 TDNETローカル検索プリセットMVP](../../decision-log/2026-09-19-tdnet-local-search-presets-mvp.md)
