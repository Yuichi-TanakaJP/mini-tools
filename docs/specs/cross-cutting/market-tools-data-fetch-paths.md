...
- `一般規制`: 一般信用 売建可だが貸株注意喚起あり
- `一般×`: 一般信用 売建規制（取引停止）。`一般規制` より重い状態
...
|---|---|---|---|---|
| `general_short=true` かつ `available_shares>0` | 一般売可 | 一般可 | Yes | Yes |
| 上記 + 貸株注意喚起 | 一般注意 | 一般規制 | Yes | Yes |
...
else if (canCrossNow && hasLendingCaution) show "一般規制";
