...
| `investor-flow` | サーバーで `loadInvestorFlowPageData()` を呼び、raw manifest / latest / week snapshot と analysis manifest / latest / week snapshot を取得する | 週切替は query string を更新して server を再評価する | `MARKET_INFO_API_BASE_URL` | なし。未設定時は「データ取得先が未接続」表示、raw 対象週 fetch 失敗時は error card 表示。analysis だけ失敗した場合は raw 表示に fallback | JPX公式データ由来の週次 API 前提。repo 同梱 JSON は持たない |
...
バッジは短語化し、`一般` 系は状態を短いラベルで表す。`制度` は一般クロス可否とは別表示にする。
ラベルの意味（詳細は `title` 属性で補足）:

- `一般可`: 一般信用 売建可（在庫あり、今クロス可）
- `一般注意`: 一般信用 売建可だが貸株注意喚起あり
- `一般停止`: 一般信用 売建規制（取引停止）。最も重い状態
- `一般×`: 一般信用売建の対象だが在庫0（今はクロス不可）
- `制度可`: 制度信用 売建可

`available_shares` の意味づけ（ライブデータ調査で確定）:

- `available_shares` が**数値（0 を含む）** = 日興が一般信用の在庫枠を管理している「一般信用売建の対象」銘柄。
- `available_shares=null` = 在庫枠データを持たない非対象。引き続き表示しない（内部監視扱い）。
- `general_short` は在庫連動で動く（在庫が尽きると `false` に倒れる）ため、**一般対象判定には使わず `available_shares` を基準にする**。`general_short ≈ (available_shares>0)` がほぼ成立し、例外は取引所規制中の銘柄のみ。

| ケース | 旧ラベル | 現ラベル | クロス対象 | 監視継続 |
|---|---|---|---|---|
| `available_shares>0`、売建規制なし | 一般売可 | 一般可 | Yes | Yes |
| 上記 + 貸株注意喚起 | 一般注意 | 一般注意 | Yes | Yes |
| 新規売建規制 + 取引停止 | 一般規制 | 一般停止 | No | Yes |
| `available_shares=0`、売建停止なし（`general_short` の真偽は問わない） | 表示なし | 一般× | No | Yes |
| `available_shares=null`、売建規制なし | 表示なし | 表示なし | No | 低頻度 |
| `institutional_short=true` | 制度売可 | 制度可 | 一般クロス判定には使わない | 任意 |
| 一般・制度どちらも不可 | 表示なし | 表示なし | No | 低頻度 |

判定順:

```ts
if (hasSellStop) show "一般停止";
else if (canCrossNow && hasLendingCaution) show "一般注意";
else if (canCrossNow) show "一般可";
else if (isOutOfStock) show "一般×"; // available_shares===0 && !hasSellStop

if (institutional_short) show "制度可";
```
...
- `isOutOfStock`: `available_shares===0` かつ売建停止なし（`general_short` は不問。在庫連動で false に倒れた在庫0銘柄を拾う）
- `3549` は `一般停止` として扱う
