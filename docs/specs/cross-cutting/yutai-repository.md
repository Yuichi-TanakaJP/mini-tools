...
2026-09-10時点で共通処理はPR593で完了。カレンダー・メモ基本編集・タグ・履歴・残高/期限の基本操作を隔離検証用に接続。
本番の保存先は変わらない。
...
| 4 既存画面接続 | 進行中。カレンダー・メモ基本編集/月別数値/タグ/履歴・残高/期限の基本操作を隔離検証接続。一括操作・入出力・ダッシュボード・期限帳の従来UI互換/スキャン・依存通知は残件 |
