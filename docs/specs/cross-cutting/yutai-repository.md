...
6グループは6PR・6ターン・同じ作業量を意味しない。4-Eは接続・合成データでの自動検証が完了し、残りは4-A/B/C/D/Fの5グループ。
...
| 4-F | 優待に依存する既存通知 | ホーム側の優待期限/仕込み関連表示を同じDB取得結果へ接続し、ログアウト/取得失敗で古い本人情報や誤った0件を表示しない。高度な新規通知は対象外 |
