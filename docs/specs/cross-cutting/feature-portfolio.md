...
1. **保有とウォッチの正本**: 現行マイ銘柄はLocalStorage。2026-09-17の調査では、取込済み保有はPortfolio snapshot/position、ウォッチ・旧メモは当面既存保存先に分ける案を推奨。採用・移行は未実施。[調査と設計提案](../../plans/portfolio-holdings-authority-design.md)を確認して判断する。
