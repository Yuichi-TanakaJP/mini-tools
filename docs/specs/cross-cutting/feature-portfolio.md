...
1. **保有とウォッチの集約**: 現行マイ銘柄はLocalStorage。2026-09-18改訂では保有を既存Portfolio、ウォッチを銘柄分析へ集約する方向。マイ銘柄は固有機能・メモ・未ログイン利用の引継ぎ条件を確認後の廃止候補であり、廃止・移行は未実施。[改訂計画](../../plans/portfolio-holdings-authority-design.md)を参照。CSV再取込や保有の二重表示を前提としない。
