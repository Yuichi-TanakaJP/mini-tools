...
- ボスごとに名前、攻撃ラベル、HP、スコア、報酬、速度補正、横移動補正を定義する
- ボス名と攻撃ラベルを画面上に表示する
...
2026-05-09 時点で、このPRはユーザー判断によりいったん凍結する。
再開条件が決まるまで、次の実装対象として自動的には進めない。
...
- 既存のステージ背景画像を活かしつつ、ゲーム画面内に多層背景を重ねる
...
- 通常小ステージにランダム障害物を追加する
- 障害物は破壊可能 / 破壊不可能の2種類にする
- 破壊可能な障害物はショットで壊せる
- 破壊不可能な障害物は避ける対象にする
- 障害物にぶつかるとライフが1減る
- 敵と一緒に出る障害物は、下端側で避けられるように上から流れる障害物として扱う
- エイリアンが出ない迷路ステージは次段階の拡張候補として残す

完了条件:

- 通常小ステージで障害物が出現する
- 破壊可能 / 破壊不可能の差が画面上と挙動で分かる
- 障害物衝突でライフが減る
- ステージ切替、ボム、ゲームリセットで障害物状態が破綻しない
...
- ボスのHPバーと演出がステージ背景やHUDと重ならない
- 障害物が敵や弾と一緒に出ても操作不能な詰み配置にならない
...
- 関連 decision-log:
  ...
  - `docs/decision-log/2026-05-10-penguin-shooter-obstacle-system.md`
