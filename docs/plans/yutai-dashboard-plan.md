...
### Phase 2: 詳細パネルからのメモ編集

- 詳細パネルでメモ本文・任期条件・早打ち目安・取得済み状態・仕込み開始を編集できるようにする（`yutai-candidates` の簡易編集パターンを流用）。

### Phase 3: 横スクロール 12 ヶ月ビュー

- タブ切替で「銘柄 × 12 ヶ月」のガント風ビューを追加する。仕込み開始月〜権利月を帯表示し、実績月に取得マークを付ける。
- 日興 / SBI のような当日変動値は 12 ヶ月グリッドに載せず、表ビュー側の担当とする。
