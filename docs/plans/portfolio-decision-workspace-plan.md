# ポートフォリオ意思決定ワークスペース 実装計画

最終更新: 2026-09-07

正本参照更新: 2026-09-14

2026-09-18追記: UI-4 / V2 Phase 6の保有統合基盤として、Portfolioと通知・銘柄分析の保有選択を共通化する。利用者指示により未同期端末メモの確認は撤去条件としない。受入条件は正本一致、Premium/RLS維持、失敗と0件の区別、旧UI通常導線の終了。詳細は[統合判断](../decision-log/2026-09-18-portfolio-holdings-authority.md)。本追記はUI-4全体やV2完成条件の変更ではない。

状態: **UI-1〜UI-2の初回実装済み。UI-3の初回範囲として、review履歴、外部資産の読み取り、公式保有と外部参照資産の総資産評価額を実装済み。`/premium/portfolio` は保存済み判断を読む意思決定ワークスペースへ移行中であり、共有読み取り契約・外部資産の商品別構成/意思決定統合・差分表示は未完成である。**

## 1. 正本と対象範囲

クロスリポジトリ全体の目的、ChatGPT運用、API、完成条件は、stock-notesの
[ポートフォリオ方針・振り返りプラットフォーム V2 正本設計](https://github.com/Yuichi-TanakaJP/stock-notes/blob/main/docs/portfolio-platform-v2.md)
を正とする。

旧 [`portfolio-platform-plan.md`](https://github.com/Yuichi-TanakaJP/stock-notes/blob/main/docs/portfolio-platform-plan.md) はV1の履歴文書であり、現在の完成条件には使用しない。実装順と受入確認は
[`portfolio-platform-v2-implementation-plan.md`](https://github.com/Yuichi-TanakaJP/stock-notes/blob/main/docs/portfolio-platform-v2-implementation-plan.md) と
[`portfolio-platform-v2-checklist.md`](https://github.com/Yuichi-TanakaJP/stock-notes/blob/main/docs/portfolio-platform-v2-checklist.md) を参照する。

この文書は、正本をMiniToolsの画面、データ取得、銘柄ダッシュボード連携、UATへ落とす実行計画である。

## 2. MiniToolsの役割

MiniToolsはポートフォリオ方針の主入力画面ではない。主入力はChatGPTとの会話であり、MiniToolsは
保存された判断を継続利用するための意思決定ワークスペースである。

この画面のWhatは、保有表を表示することではなく、ChatGPTで保存した方針・現在地・補強方向・
振り返り・次の確認事項を手入力なしで確認できることである。方針の変更やreviewの作成は、
ユーザーがGPTで相談し、確認した結果をstock-notes API/DBへ保存する。

ユーザーが画面を開いたとき、最初に答えられるべき質問は次のとおり。

- 今のポートフォリオの強みと弱みは何か
- どの領域を補強し、どの条件で待つのか。金額配分を求めた場合は、いくら入れるのか
- 今すぐ買わない銘柄は、なぜ待つのか
- どの銘柄の分析が古く、何を再確認すべきか
- 前回の方針から何が変わったか
- 次に実行・監視するアクションは何か

保有数量や損益の表は必要だが、上記判断を支える補助情報として扱う。

## 3. 現状とギャップ

### 現在実装済み

- `/premium/portfolio` のPremium認証
- Supabase Auth/RLSによる本人データの読み取り
- 最新ready snapshotの評価額・取得額・損益表示
- 口座別position、snapshot履歴の表示
- 保存済みreview/itemの読み取り表示
- 保存済みrecommendation/actionの読み取り表示（MiniToolsからの書き込みは行わない）
- 「意思決定」画面で、判断状態・全体要約・金額なしの補強/調査候補・次のActionを表示
- DBのactive policy・policy履歴をreviewの一時メモと分離して読み取り表示し、reviewが参照したpolicy versionも表示
- policy履歴とlatest reflectionの読み取り表示
- 外部口座・iDeCo・企業型DC・海外口座・暗号資産・現金等を `external_reference` snapshotとして公式保有と分けて読み取り表示
- 外部資産の未登録・取得済み・取得失敗・評価額未取得を区別して表示
- 公式保有と外部参照資産の評価額を総資産評価額として合算し、scope別内訳・基準日・状態を表示
- 最新readyの公式snapshotについて、資産クラス・口座種別・業種・景気感応度・インカム特性・時価総額帯・銘柄別の構成を、全資産／個別株で切り替えて表示

### 未実装

- ChatGPT相談開始・継続導線
- stock-notesの`GET /portfolio/decision-context`をMiniTools用の共有読み取り契約として利用すること
- 全体診断、弱み、不足する役割
- 金額配分を依頼した場合の新規資金配分・待機資金と、通常の見送り理由
- 分析鮮度、決算後未分析、取得失敗の表示
- portfolio action
- reviewとsnapshotの鮮度差
- 前回reviewとの差分
- 銘柄ダッシュボードとの双方向連携
- 外部資産をdecision context・商品別構成・配分分析へ統合すること（通貨換算、重複排除、分析対象外ルールが未定義）

現行画面を「表の見栄えを改善する」だけでは、目標は達成しない。

## 4. 標準ユーザーフロー

### 4.1 最初の棚卸し

1. 証券会社CSVがstock-notesへ取込済み
2. ユーザーがMiniToolsを開く
3. review未作成を検知し、保有件数・基準日と「ChatGPTで棚卸しを始める」を表示
4. ユーザーがカスタムGPTで相談する
5. GPTがcontextを読み、全体方針と銘柄別判断を提示
6. ユーザー確認後、GPTがreviewを保存・確定
7. MiniTools再読込で意思決定画面へ反映

### 4.2 追加資金の相談

1. 「100万円を追加したい」等をChatGPTへ伝える
2. GPTが最新snapshot、最新分析、前回reviewを取得
3. 資金配分、待機資金、条件、理由を提示
4. 修正後にreviewを保存
5. MiniToolsで配分合計と優先順位を確認

### 4.3 再評価

1. 新しいCSV、決算、分析、条件成立のいずれかが発生
2. MiniToolsに「要再レビュー」と対象理由を表示
3. ChatGPTで再相談
4. 新reviewを保存
5. 前回との差分をMiniToolsで確認

## 5. 情報設計

主ナビゲーションは次の3画面とする。

1. **意思決定**: active policy、全体判断、補強方向、アクション、最新reflection
2. **保有一覧**: 商品別集約、口座別明細、取得・評価情報
3. **履歴**: snapshot、review、前回との差分

現在の「表示・記録・方針」はデータ分類であり、ユーザーの目的を表していないため、段階的に意思決定・保有一覧・履歴へ整理する。

外部資産はポートフォリオ全体の正式な管理対象であり、保有一覧の総資産評価額へ合算する。scope別内訳と基準日は残し、商品別構成・意思決定contextへの統合は別工程とする。MiniToolsからの登録入力は追加せず、ChatGPT/APIを保存経路とする。

## 6. 意思決定画面

### 6.1 ヘッダー

- ポートフォリオ名
- snapshot基準日・取込日時
- review基準日・状態
- `最新`、`要再レビュー`、`分析不足`等の状態
- ChatGPTで相談を開始・継続するボタン

状態判定例:

| 条件 | 表示 |
|---|---|
| reviewなし | 最初の棚卸しが必要 |
| snapshot.as_of > review.as_of | 保有状況更新後、未レビュー |
| 決算日 > source analysis.as_of | 決算後未分析 |
| 未分析instrumentあり | 分析不足N件 |
| context取得失敗 | データ取得失敗。空ポートフォリオとは区別 |

### 6.2 全体判断

- 今回の結論
- ポートフォリオの強み
- 弱み・集中リスク
- 不足する役割
- 今回の大方針
- 判断に使ったデータ基準日

文章を一段落で埋めず、「結論」「理由」「リスク」「次回確認」に分ける。

### 6.3 補強候補と、依頼時の新規資金投入計画

通常は金額を仮定せず、補強する領域・既存銘柄・新規候補・テーマ・待機の理由を比較する。
ユーザーが金額配分を明示的に求めた場合は、次の項目を表示する。

優先順位順に次を表示する。

| 項目 | 内容 |
|---|---|
| 順位 | 新規資金の優先順位 |
| 商品 | コード、名称、現在構成比 |
| 提案 | 投入金額、投入後比率 |
| 判断 | 買い増し、押し目待ち、維持、縮小等 |
| 条件 | 価格、利回り、PER、決算等 |
| 理由 | portfolio上で今増やす理由 |
| 待つ理由 | 条件未成立、分析不足、集中等 |
| 鮮度 | 最終分析、次回/前回決算 |

画面上部に必ず次を表示する。

- 想定新規資金
- 提案配分合計
- 待機資金
- 配分未確定額

### 6.4 ポートフォリオ診断

- 役割別構成
- 資産種別・口座別構成
- 上位銘柄集中
- 役割の不足・重複
- 未分析・分析期限切れ・条件未設定の件数

グラフは判断を補助する場合だけ使用し、グラフ自体を成果にしない。

### 6.5 アクション

- 対象
- やること
- 期限または条件
- 状態
- 発生元review
- 完了後の結果分析

## 7. 保有一覧

### 7.1 商品別集約

- コード・名称・資産種別
- 合計数量、平均取得単価、現在値
- 取得額、評価額、損益、構成比
- 保有口座
- role、stance、need、priority
- 買い増し条件、次回アクション
- 分析鮮度

国内株は銘柄ダッシュボードへdeep linkする。投資信託は個別株分析へ強制リンクしない。

### 7.2 口座別明細

監査・確認用の副表示とする。NISAと課税口座の同一銘柄を失わない。

## 8. 履歴画面

- snapshotの基準日、取込日時、position件数
- reviewの基準日、状態、対象snapshot
- 前回からの順位、配分、stance、needの変更
- 追加・削除・数量変化したposition
- 変更理由、参照analysis/thesis

## 9. 銘柄ダッシュボード連携

### Portfolio → Stock Notes

- 国内株行から該当銘柄を展開した状態で開く
- 未分析・決算後未分析なら理由を引き継ぐ
- 分析用プロンプトまたはChatGPT相談導線を表示する

### Stock Notes → Portfolio

銘柄詳細へ以下を追加する。

- 保有数量、平均取得単価、評価額、構成比
- 最新portfolio reviewのrole、stance、need、priority
- 買い増し条件、提案金額、変更理由
- portfolio基準日、reviewへのリンク

個別thesisの強気・弱気と、portfolio上の追加優先度は別のラベルとして表示する。

## 10. データ取得方針

MiniToolsはSupabase Auth/RLSを使って本人データを読む。表示用の単純な読み取りは直接取得できるが、
判断文脈の定義はstock-notes `GET /portfolio/decision-context` と一致させる。

同じ集計・鮮度判定をMiniToolsとstock-notesで別実装して不一致にしない。次のいずれかで共有する。

1. stock-notes decision contextをMiniToolsサーバー経由で取得する
2. DB view/RPC等で正規化した共通読み取り契約を作る

採用方式はcontext API PRで、認証、応答時間、キャッシュ、障害分離を比較して決定する。

## 11. 実装PR計画

### UI-0: 文書と現状ラベル

- 本計画を正本計画へ接続
- 現行仕様に読み取り版・未完成と明記
- 旧decision logを置換せず、後続判断で更新されたことを記録

### UI-1: 読み取りcontractと型（初回実装）

- context/review/action/diffの画面用型
- 正常、reviewなし、分析不足、失敗のfixture
- データ変換の単体テスト
- recommendation/actionのSupabase読み取り型と正規化
- 金額なしrecommendationを金額配分と誤認しない表示状態

### UI-2: 意思決定画面（初回実装）

- ヘッダー、全体判断、補強候補、依頼時の投入計画、診断、アクション
- reviewなし・要再レビュー状態
- 保存済みテーマ推薦と未完了Action
- 新規資金額未設定時の明示
- active policyとreviewの一時判断を分離
- policy履歴とlatest reflectionの表示
- モバイル/PC UAT

### UI-3: 保有・履歴

- 商品別集約、口座別明細
- snapshot/review履歴と差分
- `superseded` reviewを現行判断から除外し、履歴画面には表示する
- 現行reviewがない場合に、置換済み履歴とChatGPTでの次の操作を表示する

UI-3の初回受入範囲は、stock-notes側のreview lifecycle（`draft` / `finalized` / `superseded`）とMiniToolsの表示を一致させることとする。`superseded`は未確定のまま新しいreviewへ切り替えた履歴であり、現行reviewや未完了Actionの対象にはしない。

### UI-4: 銘柄ダッシュボード統合

- 双方向deep link
- 銘柄詳細のportfolio context
- 分析鮮度・再分析導線

各PRはstock-notesの`docs/portfolio-platform-v2-checklist.md`に対応するPhaseと受入条件をPR本文へ記載する。

## 12. UAT計画

実装完了はサンプル表示ではなく、実データで次を確認する。

- 49position、39instrument、5口座が欠落なく表示される
- INPEX 1605が96株、取得単価1,927円として表示される
- ChatGPTで保存したreviewが手入力なしで反映される
- 100万円の配分を依頼した場合、提案配分合計と待機資金が一致する
- INPEXの個別評価と追加優先度が別表示される
- reviewより新しいsnapshotで要再レビューになる
- 未分析、決算後未分析、API失敗を空状態と誤認しない
- portfolioと銘柄ダッシュボードを往復できる
- 前回reviewとの差分を確認できる

実装PRごとに [`docs/uat/portfolio.md`](../uat/portfolio.md) を更新し、実施証跡はPR本文へ残す。

## 13. 完成条件

MiniTools側は、次が成立したときだけ完成とする。

> ChatGPTで保存したポートフォリオ判断を、手入力なしで読み取り、方針、現在地、補強方向、
> 待つ条件、不足する分析、実行すべきアクション、振り返り、前回との差分を確認でき、個別銘柄分析へ往復できる。
