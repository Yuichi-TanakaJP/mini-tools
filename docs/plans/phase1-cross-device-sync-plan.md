...
- **段階**: 当初は **優待メモ帳（`yutai_memo_items_v1`）1 本**を縦に通す。2026-06-21 の本番確認後、手動保存/復元の対象を `lib/sync/registry.ts` に登録した LocalStorage 系主要キー（優待メモ帳、優待カレンダー、株主優待期限帳、マイ銘柄）へ拡張した。2026-06-22 以降、ログイン直後の自動同期は止め、Phase 1 は手動保存/復元を基本にする。汎用 key-value 構造にし、後続ツールは行を足すだけ。
...
- Phase 1 では各ツールの保存を契機に自動 POST しない。ツール内保存は従来どおり LocalStorage へ即時保存し、クラウド保存は `/account` の手動操作に限定する。
- Phase 1 ではログイン直後に自動 GET/POST しない。ログインは認証状態を作るだけにし、保存/復元は `/account` の手動ボタンから実行する。
- 初回ログイン時のローカルデータ吸い上げは、自動ではなく手動の「この端末を保存」で行う。空データではクラウドを上書きしない。
