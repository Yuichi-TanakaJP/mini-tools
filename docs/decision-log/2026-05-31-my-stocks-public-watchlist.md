...
## 追記（2026-06-02）保有メモの口座区分表示

NISA と特定預りが同一銘柄で混在するケースでは、コード単位に合算して memo に内訳を入れるだけだと見づらい。

- `MyStockItem` に `accountType` / `accountLabel` を追加する。
- 保有メモでは同一コードでも口座区分が違えば別行として扱う。
- ウォッチは従来どおり同一コードを重複スキップする。
- import の非破壊マージも、保有メモは `tab + code + accountType` を重複判定キーにする。
- 既存データは口座未設定として読み続けられるよう、正規化時に欠損を許容する。
