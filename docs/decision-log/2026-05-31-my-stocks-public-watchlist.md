...
## 追記（2026-06-04）保有メモの口座別表示と比率表示

同一銘柄を特定預りと NISA の両方で持つケースでは、単純な追加順一覧だけだと内訳を確認しにくい。

- 保有メモに `一覧` / `口座別` / `比率` の表示モードを追加する。
- `口座別` は銘柄ごとのカードにまとめ、その中へ `特定` / `NISA` / `その他・未設定` の口座内訳を並べる。
  - 同一銘柄を特定預りと NISA の両方で持っている場合も、銘柄カード内でまとまって見えるようにする。
  - 内部データでは `nisa-growth` / `nisa-tsumitate` / `old-nisa` を保持し、内訳の色分けや比率集計では `NISA` として扱う。
- 一覧・口座別の行には `追加先口座` の選択と `追加` ボタンを出し、同じ銘柄を検索し直さなくても口座内訳を増やせるようにする。
- 追加先口座は既にその銘柄で登録済みの口座区分を候補から外し、追加前にユーザーが選べるようにする。
- `比率` は公開版の責務に合わせ、現在値・評価額ではなく `数量 * 取得単価` の取得額ベースで表示する。
  - ラベルは「取得額比率」とし、「評価額比率」と誤認させない。
  - 数量または取得単価が未入力の行は比率計算から除外し、除外件数を表示する。
- 個人データは引き続き localStorage 完結で、比率計算もクライアント内で行う。
