...
| Cross-cutting Spec | `docs/specs/cross-cutting/*.md` | 複数ツールにまたがる安定仕様 | URL、データ取得、SSR、共有導線などを横断整理するとき |
| Plans | `docs/plans/*.md` | 計画、検証方針、移行プラン | 将来作業や段階移行を整理するとき |
| Backlog | `docs/backlog/*.md` | 未着手候補、PR 候補、検討メモ | 仕様未確定の改善候補を残すとき |
...
複数ツールにまたがる長期参照の仕様は `docs/specs/cross-cutting/` に置く。
...
未着手候補、PR 単位候補、仕様未確定の検討メモは `docs/backlog/` に置く。

向いている内容:

- 将来やりたい改善候補
- PR 分割案
- 実装前に検討が必要な UI / data model メモ
...
- 横断仕様を追加したら、必ず `docs/specs/cross-cutting/index.md` にリンクする
- plan を追加したら、必ず `docs/plans/index.md` にリンクする
- backlog を追加したら、必ず `docs/backlog/index.md` にリンクする
- `docs/` 直下に新しい恒久 docs を置かない。直下に置くのは `index.md`、`product-spec.md`、`docs-writing-workflow.md` のような入口・運用中核だけにする
...
4. 計画なら Plans、未確定候補なら Backlog で足りるか
5. 一時メモなら Dev Log で足りるか
6. 新規ファイルにする場合、どの index から辿れるようにするか
...
- 「これからどう進めるか」を残したいものは Plans
- 「まだ確定していない候補」は Backlog
