...
- 実行可能な環境はVercel Productionビルドの `https://mini-tools-rho.vercel.app` から期待するSupabase project `uqnkjitvuebwhjvmaddb` へ接続する場合のみ。Preview・localhost・別origin・別Supabase projectでは、認証通信より前に拒否する。Previewでボタンを押してもRPC GETが発生しないことを確認する。ビルド時に `VERCEL_ENV` がない場合も拒否する。
...
- HTTP status、時刻、受信した応答バイト列のSHA-256、軽量事前チェックを記録する。保存ファイルは同じバイト列から作る。200以外、事前チェック失敗、通信失敗は未合格。200でも完全validatorを通すまで未合格。
...
- PWAでは `Authorization` または `apikey` ヘッダー付きGETを最優先のNetworkOnly規則に通し、Service WorkerのCacheStorageへ保存しない。ビルド後の `public/sw.js` でこの規則がcross-originの既定cache規則より先にあることを確認する。過去のキャッシュ内容を自動削除する変更ではない。
- 「1回」はページの1 mount中の最大回数であり、再読み込み・別タブでの再実行を禁止する仕組みではない。実行記録と運用で1件に限定する。
