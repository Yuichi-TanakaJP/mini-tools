...
export type DashboardFetchers = {
  ...
  /** 次回決算日の取得（/api/stock-notes/earnings）。失敗はダッシュボード全体を失敗させない（下記参照）。 */
  fetchEarnings: () => Promise<StockNotesEarningsInfo>;
};
...
export async function loadDashboardData(fetchers: DashboardFetchers): Promise<LoadResult> {
  try {
    const [stocks, analyses, theses, actions, holdingsResult, earnings] = await Promise.all([
      fetchers.fetchStocks(),
      fetchers.fetchAnalyses(),
      fetchers.fetchTheses(),
      fetchers.fetchOpenActions(),
      fetchers.fetchHoldings(),
      fetchers.fetchEarnings().catch(() => null),
    ]);
    return {
      status: "ok",
      stocks,
