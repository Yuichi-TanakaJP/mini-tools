...
export default function YutaiExpiryApp({ scanEnabled = false }: { scanEnabled?: boolean }) {
  ...
  const openHistory = (item:YutaiBenefitSummaryItem) => {
    setHistoryFocus({title:item.title,benefitKey:item.group_key});
    setView("history");
  };
  return (
    <main className={styles.page}>
      <header className={styles.top}>
        <div>
          <h1>株主優待期限帳</h1>
          <p>期限・優待一覧・履歴・全体実績・管理を、同じデータから別の見方で確認します。</p>
        </div>
      </header>
        ...
        {view === "management" && (
          <section className={styles.management}>
            <div className={styles.managementIntro}>
              <h2>管理</h2>
              <p>Lot、Account、状態確認、旧台帳、撮影・画像取込などを扱います。時系列は「履歴」、優待ごとの通算は「優待一覧」、全体の金額実績は「実績」で確認できます。</p>
            </div>
