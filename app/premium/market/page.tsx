...
export const metadata: Metadata = {
  title: "業種トレンド (TOPIX33) | mini-tools premium",
  description: "TOPIX33 業種の月内モメンタムを、ヒートマップと業種比較チャートで確認する premium ページです。",
  alternates: {
    canonical: "/premium/market",
  },
};
...
const monthNavLinkStyle: CSSProperties = {
  display: "inline-flex",
  alignItems: "center",
  padding: "8px 14px",
  borderRadius: 999,
...
export default async function PremiumMarketPage({
  searchParams,
}: {
  ...
  if (!verifyPremiumSession(session)) {
    const nextPath = month
      ? `/premium/market?month=${encodeURIComponent(month)}`
      : "/premium/market";
    redirect(`/premium/login?next=${encodeURIComponent(nextPath)}`);
  }

  const preview = await loadPremiumPreviewData(month);
    ...
    <main style={{ padding: "28px 16px 72px" }}>
      ...
      >
        <nav style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
          <Link
            href="/premium"
            style={{ color: "var(--color-text-sub)", textDecoration: "none", fontSize: 13, fontWeight: 800 }}
          >
            ← Premium ホーム
          </Link>
          <Link
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 18 }}>
            <div>
              <div style={eyebrowStyle}>月間ヒートマップ</div>
        ...
        >
          <div style={eyebrowStyle}>モメンタム要約</div>
